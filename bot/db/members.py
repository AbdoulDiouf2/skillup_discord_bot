...
async def set_thread_objectif_id(
    db: aiosqlite.Connection, member_id: int, thread_id: str | None
) -> None:
    await db.execute(
        "UPDATE members SET thread_objectif_id = ? WHERE id = ?", (thread_id, member_id)
    )
    await db.commit()
